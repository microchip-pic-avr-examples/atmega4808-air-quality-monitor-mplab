...
void ParameterInit (void)
{
    ...
    readByte = NVM_ReadEepromByte (SYNC_BYTE_ADDRESS); //reading byte from internal EEPROM , address = SYNC_BYTE_ADDRESS

    if (SYNC_BYTE != readByte)
    {
        //first time power on
        NVM_WriteEepromBlock (EXTERNAL_EEPROM_WRITE_INDEX_ADDRESS, (uint8_t *) & firstEepromWriteAddress, 4); //writing first time write address (of external eeprom) in internal eeprom
        NVM_WriteEepromBlock (EXTERNAL_EEPROM_SEND_INDEX_ADDRESS, (uint8_t *) & firstEepromSendAddress, 4); //writing first time send address (of external eeprom) in internal eeprom
        NVM_WriteEepromByte (SYNC_BYTE_ADDRESS, SYNC_BYTE); //writing SYNC_BYTE in internal eeprom 
    }
...
void ResetWriteAddress (void)
{
    NVM_WriteEepromByte (SYNC_BYTE_ADDRESS, RESET_BYTE); //writing RESET_BYTE in internal eeprom 
}
