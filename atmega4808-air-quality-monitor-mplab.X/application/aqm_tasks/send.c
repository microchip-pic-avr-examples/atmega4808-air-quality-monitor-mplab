 ...
 *****************************************************************************/
timerStruct_t SEND_TasksTimer = {SEND_Task};
...
//        AqmPrinter(DEBUG_AQM, "in GET_WRITE_INDEX send task");
        NVM_ReadEepromBlock (EXTERNAL_EEPROM_WRITE_INDEX_ADDRESS, (uint8_t *) & eepromWriteIndex, LENGTH_OF_EEPROM_ADDRESS); //read(from internal eeprom) the address to which data has to be write in external eeprom 
        NVM_ReadEepromBlock (EXTERNAL_EEPROM_SEND_INDEX_ADDRESS, (uint8_t *) & eepromSendIndex, LENGTH_OF_EEPROM_ADDRESS); //read(from internal eeprom) the address fromm which data has to be send in external eeprom 
...
//            AqmPrinter (INFO_AQM, "send index = %lu", eepromSendIndex);
    ...
    case UPDATE_SEND_INDEX:
        ...
        NVM_WriteEepromBlock (EXTERNAL_EEPROM_SEND_INDEX_ADDRESS, (uint8_t *) & eepromSendIndex, LENGTH_OF_EEPROM_ADDRESS);
        task = GET_SEND_INDEX;
